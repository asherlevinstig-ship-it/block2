...
for(const id in BLOCKS){
  const bid=+id;
  if(bid===B.WATER||bid===B.LAVA||bid===B.BEDROCK||bid===B.BARRIER||bid===B.FARMLAND||bid===B.WHEAT_1||bid===B.WHEAT_2||bid===B.WHEAT_3) continue;
...
function buildRegionalLandmarks(setBlock,getBlock){
  ...
  for(const s of specs){const x=s.x,y=s.y,z=s.z;
    ...
    }else if(s.type==='abandoned_tower'){prep(s,8,B.COBBLE);for(let k=1;k<=18;k++)for(let ox=-5;ox<=5;ox++)for(let oz=-5;oz<=5;oz++)if(Math.abs(ox)===5||Math.abs(oz)===5){if(!(oz===5&&Math.abs(ox)<=1&&k<=3)&&hash2(x+ox+k,z+oz-k)>.08)setBlock(x+ox,y+k,z+oz,k%3?B.COBBLE:B.BRICK);}box(x-6,y+18,z-6,x+6,y+18,z+6,B.PLANKS);for(const [ox,oz] of [[-6,-6],[6,-6],[-6,6],[6,6]])setBlock(x+ox,y+19,z+oz,B.LANTERN);
...
function generateWorld(){
  ...
  regionalLandmarks=buildRegionalLandmarks(setB,getB);
...
const tutorialBuildAreaBeam=new THREE.Mesh(new THREE.BoxGeometry(3.08,40,3.08),new THREE.MeshBasicMaterial({color:0x8ee8ff,transparent:true,opacity:.12,depthWrite:false,depthTest:false,side:THREE.DoubleSide,blending:THREE.AdditiveBlending}));
tutorialBuildAreaBeam.position.y=15.97;tutorialBuildAreaBeam.renderOrder=90;tutorialBuildAreaBeam.frustumCulled=false;tutorialBuildAreaBeam.visible=false;
const tutorialBuildAreaFloor=new THREE.Mesh(new THREE.PlaneGeometry(3.08,3.08),new THREE.MeshBasicMaterial({color:0xb8ff8a,transparent:true,opacity:.42,depthWrite:false,depthTest:false,side:THREE.DoubleSide,blending:THREE.AdditiveBlending}));
tutorialBuildAreaFloor.rotation.x=-Math.PI/2;tutorialBuildAreaFloor.position.y=-4.03;tutorialBuildAreaFloor.renderOrder=91;tutorialBuildAreaFloor.visible=false;
tutorialPillarGroup.userData.buildAreaBeam=tutorialBuildAreaBeam;tutorialPillarGroup.userData.buildAreaFloor=tutorialBuildAreaFloor;
tutorialPillarGroup.add(tutorialBeamAura,tutorialBeam,tutorialTargetDisc,tutorialRing,tutorialBuildAreaBeam,tutorialBuildAreaFloor);tutorialPillarGroup.visible=false;scene.add(tutorialPillarGroup);
