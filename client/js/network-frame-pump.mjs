...
export function createNetworkFramePump({
  ...
  return function netTick(dt,now){
    ...
    for(const sid in NET.remotes){
      ...
      r.grp.visible=ref.connected!==false&&dim!=='ability'&&(ref.dgn||'')===NET.dgn&&distSq<=PERFORMANCE_BUDGETS.playerCullSq;
      ...
      if(animateHunterIdle)animateHunterIdle(r,now,stepDt,moving,stride);
