...
function start(){if(!NET.on||!NET.room)return sysMsg('Recall Cast requires a server connection.');if(active)return sysMsg('Choose an <b>answer pillar</b>.');NET.room.send('recallStart',{yaw:player.yaw,subject:selectedSubject()});}
...
function result(m){
  ...
  if(globalThis.BlockcraftOnboarding)globalThis.BlockcraftOnboarding.markRecall();
  const answer=active&&active.answers&&active.answers[m.correctIndex]||'';
  if(m.correct){const gain=Math.max(1,Math.ceil(maxSp()*(Number(m.staminaFraction)||.2)));sp=Math.min(maxSp(),sp+gain);resultFlash();showName('+'+(m.mana|0)+' MP · +'+gain+' SP'+(m.explorationGold?' · +'+m.explorationGold+' GOLD':''));feedbackEl.textContent='Correct. '+(m.explanation||'')+' Review '+reviewTiming(m.nextDue)+'.';feedbackEl.className='correct';sysMsg('Recall reward: <b>+'+(m.mana|0)+' MP</b> and <b>+'+gain+' SP</b>'+(m.explorationGold?' plus <b>+'+(m.explorationGold|0)+' gold</b> from the ruins.':'.')+' '+escHTML(m.explanation||'')+' <b>Review '+reviewTiming(m.nextDue)+'.</b>');SFX.level();}
