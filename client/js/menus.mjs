...
const SFX=(()=>{
  ...
  function tutorialMusicMode(job, dimension=''){
    if(dimension==='tutorial'||dimension==='ability'||dimension==='job')return 'tutorial';
    if(job)return 'tutorial';
    return '';
  }
  function nextMusicMode(inMenu, inTown, inTavern, outdoor, inCutscene, inBattle, tutorialJob='', dimension='', inMeditation=false){
...
function inventorySortCategory(stack){
  ...
  if([I.COAL,I.CHARCOAL,I.IRON_INGOT,I.DIAMOND,I.WHEAT_SEEDS,I.WHEAT,I.WINDSEED,I.HEARTWOOD_RESIN,I.SUNSHARD,I.MESA_AMBER,I.FROST_CRYSTAL,I.MIRE_BLOOM,I.COMPOST,I.GOLDEN_WHEAT,I.GEODE,I.RAINWAKE_PETAL,I.STORMGLASS,I.SOLAR_GLYPH].includes(id))return 30;
...
function chestBulkMaterialItem(id){
  ...
  return [I.STICK,I.COAL,I.CHARCOAL,I.IRON_INGOT,I.DIAMOND,I.WHEAT_SEEDS,I.WHEAT,I.WINDSEED,I.HEARTWOOD_RESIN,I.SUNSHARD,I.MESA_AMBER,I.FROST_CRYSTAL,I.MIRE_BLOOM,I.COMPOST,I.GOLDEN_WHEAT,I.GEODE,I.RAINWAKE_PETAL,I.STORMGLASS,I.SOLAR_GLYPH].includes(id);
}
...
function continueOpenTransitionPanel(){
  ...
  setTimeout(()=>{
    ...
    if(S&&!S.path&&serverTutorials.onboarding>=7&&combatApi.showPathSelection)combatApi.showPathSelection();
  },80);
  return !!button;
}
function openPathRecovery(){
  if(pathChoicePanelVisible()){
    closeQWin(false);
    sysMsg('<b>Choose Path:</b> preview Shadow, Mage, Guardian, or Verdant, then confirm your permanent choice.');
    return true;
  }
  closeQWin(false);
...
function dragonLoanTimeLeftText(dueAt){
  const ms=Math.max(0,(dueAt||0)-Date.now());
  const h=Math.floor(ms/3600000),m=Math.floor((ms%3600000)/60000);
  if(h>0)return h+'h '+String(m).padStart(2,'0')+'m';
  return Math.max(0,Math.ceil(ms/60000))+'m';
}
function activeDragonLoanRows(){
  return activeDragonLoans().slice().sort((a,b)=>(a.dueAt||0)-(b.dueAt||0));
}
function appendDragonLoanStatusPanel(parent){
  const loans=activeDragonLoanRows();
  if(!loans.length)return;
  const panel=document.createElement('div');panel.className='dragon-loan-status trade-error-panel';
  const h=document.createElement('b');h.textContent='Active Dragon Training Loans';panel.appendChild(h);
  for(const loan of loans){
    const line=document.createElement('div');line.className='dragon-loan-row';
...
function openDragonBondUI(){
  ...
  qpanelEl.appendChild(intro);
  appendDragonLoanStatusPanel(qpanelEl);
...
function dragonLoanOwnedTypes(){
  ...
  return unlocks.filter(t=>DRAGON_TYPES[t]&&!unavailable.has(t)&&(!COMPANIONS.dragonIsAdult||COMPANIONS.dragonIsAdult(t)));
}
...
function applyDragonLoanPending(m){
  sysMsg('Dragon training offer sent to <b>'+escHTML(String(m&&m.toName||'Hunter'))+'</b>: '+escHTML(dragonLoanName(m&&m.type))+' for <b>'+Math.max(0,(m&&m.feeGold)|0).toLocaleString('en-US')+'g</b>.',{tier:'minor',title:'Dragon Loan'});
}
const seenDragonLoanOfferIds=new Set();
function applyDragonLoanOffer(m){
  if(!m||!m.id)return;
  const id=String(m.id);
  if(seenDragonLoanOfferIds.has(id))return;
  seenDragonLoanOfferIds.add(id);
  if(seenDragonLoanOfferIds.size>16)seenDragonLoanOfferIds.delete(seenDragonLoanOfferIds.values().next().value);
  if(statOpen){statOpen=false;statEl.classList.add('hidden');}
  if(uiOpen)closeUI(false);
...
function applyDragonLoanReturn(m){
  const loan=m&&m.loan||{};
  SFX.level();
  sysMsg('<b>'+escHTML(dragonLoanName(loan.type))+'</b> returned from training.'+(loan.role==='owner'?' You can summon it again.':' It is no longer in your borrowed dragons.'),{tier:'major',title:'Dragon Loan'});
  if(qOpen&&qMode==='management')setTimeout(openDragonBondUI,120);
}
...
function appendRoadWardenPanel(){
  ...
  safety.innerHTML='<b>Shared regional safety: '+roadSafety+'/100 · '+safetyTier+'</b><br><small>Road Warden contracts and regional trouble raise this shared score. At 65/100 the B-rank readiness target is met; higher safety also suppresses hostile patrols, improves merchant prices, and brings caravans back.</small>';
...
function jobContractStepsHTML(c){
  const steps=(JOB_SYSTEM.guideSteps?JOB_SYSTEM.guideSteps(c&&c.type):jobContractGuideLines(c)).slice(0,3);
  if(!steps.length)return '';
  return '<div class="job-contract-steps"><b>How to complete</b><ol>'+steps.map(line=>'<li>'+escHTML(line)+'</li>').join('')+'</ol></div>';
}
function chooseBeginnerRecommendedOffer(offers,offerJob){
  offers=Array.isArray(offers)?offers:[];
  if(!offers.length)return null;
  const starter=offers.find(o=>o&&o.title==="Mara's Field Work");
  if(starter)return starter;
  const quickForJob=offers.find(o=>o&&o.job===offerJob&&o.difficulty==='quick');
  if(quickForJob)return quickForJob;
...
function jobOfferCardHTML(offer,opts={}){
  ...
  return '<div class="job-offer-main">'+
    (opts.recommended?'<small class="job-offer-badge">BEGINNER RECOMMENDED</small>':'')+
