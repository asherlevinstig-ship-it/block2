...
test('terrain performance stays within frame and chunk-build budgets', async ({ page }, testInfo) => {
  ...
  await expect.poll(()=>page.evaluate(()=>window.BlockcraftGameContext.requireModule('world').pendingChunkCount()),{timeout:120000}).toBe(0);
  const result=await page.evaluate(async ()=>{
    const world=window.BlockcraftGameContext.requireModule('world');
    const results={};
    const sample=async(name,action)=>{
      world.resetChunkProfile();
      window.__BLOCKCRAFT_E2E__.resetFrameProfile();
      for(let i=0;i<60;i++){
        await new Promise(requestAnimationFrame);
        if(action)action(i);
      }
      await new Promise(requestAnimationFrame);
      results[name]={...window.__BLOCKCRAFT_E2E__.frameProfile(),...world.chunkProfile()};
    };
    await sample('idle');
