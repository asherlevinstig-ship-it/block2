...
test('Dragon Roost skyline has animated atmospheric flying dragons', () => {
  ...
  assert.match(companions, /anim\.legs\.push\(\{leg,ankle,foot,sx,sz,phase:/);
  assert.match(companions, /anim\.tail\.push\(rememberDragonPart/);
  assert.match(networking, /tickPetTamerTutorialGroundDragon/);
  assert.match(networking, /animateDragonMotion/);
  ...
  assert.match(companions, /animateDragonMotion\(rec\.dragon, now\+rec\.phase\*1000, dt, following&&walkSpeed>\.04\?'walk':'idle'/);
  assert.match(companions, /animateDragonMotion\(localMountObj, now, dt, 'mountedFlight'/);
  assert.match(companions, /animateDragonMotion\(rec\.group, now\+rec\.phase\*1000, dt, 'sky'/);
  assert.match(companions, /animateDragonMotion\(d, now\+perch\.userData\.phase\*1000, dt, baby\?'walk':'idle'/);
...
test('client renders Deity power effects and stealth shimmer states', () => {
  ...
  assert.match(frame, /function disposeWeatherDiscoveryFx\(\)/);
  assert.match(frame, /weatherDiscoveryQuietUntil=performance\.now\(\)\+9000/);
  assert.match(frame, /ud\.beam\.visible=active&&\(weatherSense\|\|dist<28\)/);
  assert.match(world, /BlockcraftExplorationFx\.weatherChanged/);
...
test('client dimensions and server consume the shared grid contract', () => {
  ...
  assert.match(authSource, /const passwordWrap = password && password\.closest \? password\.closest\('\.password-wrap'\) : null;/);
  assert.match(authSource, /const forgotPassword = typeof document === 'undefined' \? null : document\.getElementById\('forgotpass'\);/);
  assert.match(authSource, /if \(passwordWrap\) passwordWrap\.classList\.toggle\('hidden', signed\);/);
  assert.match(authSource, /if \(forgotPassword\) forgotPassword\.classList\.toggle\('hidden', signed\);/);
  ...
  assert.match(teacherHtml, /"vue":"https:\/\/esm\.sh\/vue@3\.5\.18\/dist\/vue\.esm-browser\.prod\.js"/);
  ...
  assert.match(teacherDashboardSource, /function isMisLaunch\(\)/);
  assert.match(teacherDashboardSource, /\['mis', 'assessment', 'compscigo'\]\.includes\(source\)/);
  assert.match(teacherDashboardSource, /compscigo\\.com/);
  assert.match(teacherDashboardSource, /function clearTeacherHandoffToken\(\)/);
  assert.match(teacherDashboardSource, /url\.searchParams\.delete\('source'\)/);
  ...
  assert.match(teacherDashboardSource, /MIS did not provide a teacher handoff token/);
  assert.match(teacherDashboardSource, /const usedHandoff = await consumeTeacherHandoff\(\);/);
  assert.match(teacherDashboardSource, /const signedIn = usedHandoff \|\| \(!isMisLaunch\(\) && await loadAccount\(\)\);/);
  ...
  assert.match(teacherDashboardSource, /Questions in this topic/);
  assert.match(teacherDashboardSource, /Class breakdown/);
  assert.match(teacherDashboardSource, /Question breakdown for selected class\/scope/);
  assert.match(teacherDashboardSource, /School breakdown/);
  assert.match(teacherDashboardSource, /Question breakdown by school/);
  ...
  assert.match(teacherDashboardSource, /Compare schools/);
  assert.match(teacherDashboardSource, /schoolComparisonRows/);
  assert.match(teacherDashboardSource, /School comparison/);
  assert.match(teacherDashboardSource, /questionBreakdownRows/);
  assert.match(teacherDashboardSource, /By class/);
  assert.match(teacherDashboardSource, /By year group/);
  ...
  assert.match(teacherDashboardSource, /completeCurriculumRequest/);
  ...
  assert.match(sitegroundMailBridge, /\$to = bcm_clean\(\$payload\['to'\] \?\?/);
  assert.match(sitegroundMailBridge, /Invalid recipient/);
  ...
  assert.match(stylesSource, /\.teacher-vue-attention-row/);
  assert.match(stylesSource, /\.teacher-vue-assignment-row/);
...
test('retired level two job chooser cannot open',()=>{
  ...
  assert.match(combat,/const MONK_TUTORIAL_ACTIONS=Object\.freeze\(\[/);
...
test('right-side HUD panels stack without hiding the quest-log shortcut',()=>{
  ...
  assert.match(menus,/getComputedStyle\(el\)/);
...
test('First Hands guides the player through the first real objective',()=>{
  ...
  assert.match(world,/mara-first-hands-gate/);
...
test('local quick chat remains visible in a Minecraft-style bottom-left feed',()=>{
  ...
  assert.match(css,/\.chatline\{[\s\S]*background:rgba\(2,7,14,\.82\)[\s\S]*color:#f7fbff[\s\S]*'Courier New'/);
...
test('utility ability screen explains slots, unlock sources, and gameplay use cases',()=>{
  ...
  const utilityRender=menus.slice(menus.indexOf('function renderUtilitiesUI()'),menus.indexOf('function jobContractGuideLines'));
  assert.match(menus,/utility-loadout-panel/);
  assert.match(menus,/utility-modal-shell/);
  assert.match(menus,/utility-library-list/);
  assert.match(menus,/utility-detail/);
  assert.match(menus,/data-utility-select/);
  assert.match(menus,/data-utility-equip/);
  assert.match(menus,/Shift\+1-3/);
  ...
  assert.match(styles,/\.utility-modal-shell/);
...
test('all cutscene entry points are disabled while their assets remain dormant', () => {
  ...
  assert.match(opening, /startBtn\.addEventListener\('click', startWithGesture\)/);
...
test('quest log progression director introduces one system at a time',()=>{
  ...
  assert.match(menus,/gateReadinessHints/);
